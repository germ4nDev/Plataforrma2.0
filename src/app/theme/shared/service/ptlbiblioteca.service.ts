...
export class PtlBibliotecaService {
  ...
  getBibliotecaByCode(code: string) {
    const url = `${base_url}/biblioteca/code/${code}`;
    return this.http.get(url).pipe(
      map((resp: any) => {
  ...
  actualizarBiblioteca(biblioteca: PTLBiblioteca) {
    const url = `${base_url}/biblioteca/${biblioteca.codigoBiblioteca}`;
  ...
  eliminarBiblioteca(_id: any) {
    ...
    const url = `${base_url}/biblioteca/${_id.id}`;
